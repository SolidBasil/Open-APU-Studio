# Plan: Unificación de columnas `importe`/`subtotal` → `total`
...
árbol**: jerarquía, `clave` (vínculo a insumos), `descripcion` (para
...
CREATE TABLE IF NOT EXISTS estructura_presupuesto (
    ...
    -- clave solo para conceptos → cruza contra insumos.clave_opus
    clave           TEXT,
...
- `apu_matrices.importe` — sigue siendo GENERATED, es otra tabla
...
actualizar_total(concepto_id):
  └─ para cada nodo afectado (bottom-up desde el editado hasta la raíz):
     ...
     SI el nodo es 'concepto':
       └─ buscar el insumo vinculado (ep.clave → insumos.clave_opus)
...
```sql
SELECT
    n.id, n.padre_id, n.wbs, n.nivel, n.tipo, n.clave,
...
LEFT JOIN insumos i ON i.clave_opus = n.clave AND i.proyecto_id = n.proyecto_id
...
| Archivo | Cambio |
|---------|--------|
| `backend/schema.sql` | Eliminar 5 columnas, agregar `total`, actualizar comentarios |
...
| Archivo | Cambio |
|---------|--------|
| `backend/core.py` | SQL: LEFT JOIN insumos, SELECT `n.total`, COALESCE descripción. Eliminar del SELECT: `n.descripcion_corta, n.unidad, n.precio_unitario, n.importe, n.subtotal`. Actualizar docstring del dict. |
