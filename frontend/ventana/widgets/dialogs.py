...
class DialogoSeleccionarInsumo(QDialog):
    ...
    def __init__(self, api, parent=None, *, default_tipos: set[int] | None = None,
        ...
        # Con búsqueda inicial (ver _on_fila_vacia_editada, mixins/apu.py:
        # el usuario ya escribió texto que no matcheó ningún insumo exacto)
        # no restringimos por tipo — no sabemos a qué categoría del
        # catálogo pertenece lo que escribió, así que buscamos en todas.
        if busqueda_inicial:
            self._tipos_filtro: set[int] = set()
        else:
        ...
        # Teclado-first: el foco inicia en la búsqueda; al pulsar una flecha
        # pasa a la lista de insumos, y al teclear texto regresa a la búsqueda.
        ...
        if busqueda_inicial:
            # setText() dispara textChanged -> _aplicar_filtros(); recién
            # aquí self._tree ya existe (se construyó más arriba, junto
            # con self._items). Ponerlo antes revienta con
            # AttributeError: 'DialogoSeleccionarInsumo' object has no
            # attribute '_tree'.
            self._search.setText(busqueda_inicial)
        else:
        ...
        # Esc cierra sin seleccionar (funciona con el foco en cualquier parte).
        from PySide6.QtGui import QShortcut, QKeySequence
        _sc_esc = QShortcut(QKeySequence(Qt.Key.Key_Escape), self)
        _sc_esc.setContext(Qt.ShortcutContext.WindowShortcut)
        _sc_esc.activated.connect(self.reject)

    def eventFilter(self, obj, event):
        """Foco dirigido: flechas → lista de insumos, teclear texto → búsqueda.
        Enter confirma lo seleccionado; Esc cancela."""
        from PySide6.QtCore import QEvent
        from PySide6.QtWidgets import QApplication
        if event.type() != QEvent.Type.KeyPress:
            return super().eventFilter(obj, event)
        if obj not in (self._search, self._tree):
            return super().eventFilter(obj, event)
        key = event.key()
        ...
        if obj is self._tree:
            ...
            if txt and txt.isprintable() and txt.strip():
                self._search.setFocus()
                self._search.insert(txt)
                return True
        elif obj is self._search and key in (
    ...
    def _primera_seleccionable(self):
        """Primera fila visible con id real (salta el placeholder 'nuevo…')."""
        for i in range(self._tree.topLevelItemCount()):
    ...
    def _on_aceptar(self):
        sel = self._tree.currentItem()
        if sel is None or sel.data(0, Qt.ItemDataRole.UserRole) is None:
            sel = self._primera_seleccionable()
        if not sel:
            return
...
class InsumoDialog(QDialog):
    ...
    def __init__(self, api, insumo_id: int | None = None, parent=None,
        ...
        self._avanzadas_btn.setIcon(icono("chevron-right", 12))
    ...
    def _alternar_avanzadas(self):
        ...
        self._avanzadas_btn.setIcon(
            icono("chevron-down" if not visible else "chevron-right", 12))
        self.adjustSize()
    ...
    def _guardar(self):
        ...
        if unidad and unidad not in UNIDADES:
            ...
            if not confirmar(
                ...
                "Guardar de todas formas", destructivo=False,
            ):
                return
