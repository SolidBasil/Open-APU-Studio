...
def _editable_cols_detalle(item):
    # Fila vacía final (EMPTY_ROLE): solo Descripción, para buscar un
    # insumo del catálogo y agregarlo como componente — ver
    # TablaApuDetalle._crear_editor_descripcion / _on_fila_vacia_editada.
...
class TablaApuDetalle(TreeTableWidget):
    ...
    def __init__(self, matriz_id: int, descripcion: str = "", on_apu_click=None, parent=None):
        ...
        super().__init__(
            ...
            column_editors={2: self._crear_editor_descripcion, 3: _combo_unidad, 5: _combo_operador},
    ...
    def _add_empty_row(self):
        ...
        item.setToolTip(2, "Escribe el nombre de un insumo del catálogo "
                            "para agregarlo como componente nuevo")
        self._estilizar_fila_vacia(item)

    def _al_click_fila_vacia(self):
        """No-op: con la fila vacía editable, un clic simple solo
        selecciona la celda — ya no abre un diálogo modal aparte.
        Escribir en Descripción es lo que agrega el componente (ver
        _on_fila_vacia_editada)."""
        pass

    def _crear_editor_descripcion(self, parent):
        """Editor de la columna Descripción (2) de la fila vacía final:
        QLineEdit con autocompletado contra las descripciones del
        catálogo de insumos — mismo patrón que TablaArbol (Presupuesto),
        ver arbol.py::_crear_editor_descripcion."""
        ...
        _QTimer.singleShot(0, editor.selectAll)
        return editor

    def _on_fila_vacia_editada(self, item, column):
        """Escribir en la fila vacía final agrega un componente nuevo al
        APU — comportamiento tipo Excel, mismo criterio que
        TablaArbol._on_fila_vacia_editada / _on_insumo_fila_vacia_editada
        (paneles.py): un componente siempre referencia un insumo real del
        catálogo, nunca texto libre.

          - Descripción EXACTA (sin importar mayúsculas) de un único
            insumo del catálogo → se agrega directo, sin diálogo.
          - Cualquier otro caso → se abre el selector de siempre, con el
            texto ya precargado como búsqueda.

        No hace falta convertir el item a mano: apu_agregar_componente()
        dispara ApuComponenteActualizado/ProyectoRecalculado, y
        _on_evento (conectado a EVENTOS_SUSCRITOS) ya repuebla la tabla
        completa — eso regenera la fila vacía sola.
        """
        if column != 2:
            return
        api = getattr(self, "_api", None)
        ...
        win = self.window()
        handler = getattr(win, "_on_agregar_componente_apu", None)
        if handler is None:
            return
        if insumo_id is not None:
            handler(self._matriz_id, insumo_id=insumo_id)
        else:
            handler(self._matriz_id, busqueda_inicial=texto)
    ...
    def _on_item_editado(self, item, column):
        ...
        if column == 4:
            ...
            try:
                ...
                # el texto inválido tecleado ("no-es-un-numero") mientras
                # la BD seguía con el precio anterior — pantalla y BD
                # desincronizadas, sin ningún aviso de que no se guardó.
                QMessageBox.warning(self.window(), "Precio inválido",
