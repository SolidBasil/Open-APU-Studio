...
def _tooltip_rico(texto: str) -> str:
    """Envuelve `texto` en HTML simple para que el tooltip haga salto de
    línea automático en vez de una sola línea horizontal.

    Con setToolTip(col, texto_plano), Qt muestra el tooltip en UNA sola
    línea sin importar qué tan largo sea el texto — para una descripción
    corta se ve bien, pero para una descripción de 150+ caracteres el
    tooltip termina siendo una franja horizontal casi tan ancha como la
    pantalla, y a veces se recorta contra el borde: el ancho visual del
    tooltip queda inconsistente según cuánto mida cada descripción. Al
    envolver el texto en un <div> con max-width, Qt lo detecta como rich
    text y aplica ajuste de línea automático dentro de ese ancho — el
    tooltip queda con un ancho máximo consistente sin importar la longitud
    del texto, partiéndose en varias líneas cuando hace falta.
    """
    import html
    escapado = html.escape(str(texto)).replace("\n", "<br>")
    return f'<div style="max-width:420px; white-space:normal;">{escapado}</div>'
...
class PersonalizarColumnasDialog(QDialog):
    ...
    def _pintar_star(self, star: QLabel, es_fav: bool):
        """Estrella favorita como icono SVG (sin depender de glifos de fuente)."""
        star.setPixmap(_icono("star", 18, "#F0C060" if es_fav else MUTED).pixmap(18, 18))
        star.setStyleSheet(
            "QLabel { padding: 2px 4px; border-radius: 3px; }"
...
class BotonCerrarTab(QToolButton):
    """Botón de cierre de pestaña dibujado con QPainter.

    Deliberadamente NO pasa por el pipeline SVG/tint/DPR de iconos.py:
    dos líneas rasterizan nítidas a cualquier escala del sistema (el
    QPainter rasteriza a la resolución física del dispositivo de destino)
    y el color sigue el tint del tema activo via get_default_tint(), que
    la toolbar actualiza en cada cambio de modo.
    """
    ...
    def __init__(self, parent=None):
        ...
        # Reset por-widget: la regla global QToolButton { min-width:48px;
        # min-height:40px; padding } de los temas ganaría sobre setFixedSize.
        self.setStyleSheet(
            "QToolButton#tabCloseBtn { min-width: 0px; min-height: 0px;"
            " max-width: 16px; max-height: 16px; padding: 0px;"
            " background: transparent; border: none; }"
        )
        self.setFixedSize(16, 16)

    def paintEvent(self, event):
        from frontend.ventana.iconos import get_default_tint
        tint = QColor(get_default_tint())
        p = QPainter(self)
        ...
        p.setPen(pen)
        m = 4  # X de 8x8 dentro del botón de 16x16
        p.drawLine(m, m, self.width() - m, self.height() - m)
        p.drawLine(self.width() - m, m, m, self.height() - m)
        p.end()


class TabWidgetCerrable(QTabWidget):
    """QTabWidget con botón de cierre por pestaña (BotonCerrarTab).

    Reemplaza el close-button por defecto del estilo (que queda invisible
    bajo los temas QSS del proyecto). Al pulsar emite tabCloseRequested(idx);
    el índice se resuelve en el momento del clic (via indexOf de la página),
    así que no se desincroniza si el usuario reordena o cierra pestañas.
    """

    def tabInserted(self, pos: int):
        super().tabInserted(pos)
        self._instalar_boton_cerrar(pos)

    def _instalar_boton_cerrar(self, pos: int):
        pagina = self.widget(pos)
        if pagina is None:
            return
        btn = BotonCerrarTab(self)
        btn.clicked.connect(
            lambda _c=False, tw=self, p=pagina: TabWidgetCerrable._pedir_cerrar(tw, p))
        self.tabBar().setTabButton(pos, QTabBar.ButtonPosition.RightSide, btn)

    @staticmethod
    def _pedir_cerrar(tw: "TabWidgetCerrable", pagina: QWidget):
        idx = tw.indexOf(pagina)
        if idx >= 0:
            tw.tabCloseRequested.emit(idx)
...
class _Delegate(QStyledItemDelegate):
    ...
    def updateEditorGeometry(self, editor, option, index):
        """Editor a celda completa, estilo Excel: cubre todo el rect de la
        celda, sin reducirse ni dejar márgenes (el QLineEdit se estiliza
        sin marco ni padding en createEditor — el QSS global de QLineEdit
        tiene border+padding que lo encogería)."""
    ...
    def createEditor(self, parent, option, index):
        ...
        if col in self._column_editors:
            ...
            if isinstance(editor, QLineEdit):
                # Mismo estilo celda completa para los editores custom
                # (ej. _crear_editor_descripcion con autocompletado).
        ...
        if editor:
            ...
            if isinstance(editor, QLineEdit):
                # Estilo celda completa (misma técnica que
                # structural_studio_v2/sidebar_estructura.py): sin marco,
                # fondo transparente y sin padding — el QSS global de
                # QLineEdit (border 1px + radius + padding 4x8) lo haría
                # verse reducido dentro de la celda.
                editor.setFrame(False)
                editor.setStyleSheet(
                    "QLineEdit { background: transparent; border: none;"
                    " padding: 0px; margin: 0px; }"
    ...
    def setEditorData(self, editor, index):
        ...
        if isinstance(editor, QComboBox):
            ...
            editor.selectAll()  # tras setText: selección Excel de toda la celda
        else:
...
class TreeTableWidget(QTreeWidget):
    ...
    def __init__(self, columns, editable_cols=frozenset(), flat=False,
        ...
        self._column_map: dict[int, ColumnaDef] = {c.idx: c for c in self.COLUMNAS_CATALOGO}
        self._corte_pendiente: set[tuple] | None = None  # {(item, col), ...} — ver _cut()
        self._press_pos = None  # posición del último mousePress — ver _puede_iniciar_drag
        self._press_col = 0
        self._rubber_origin: QPoint | None = None  # inicio de rubber band — ver mousePressEvent
        self._rubber_band: QRubberBand | None = None
        self._press_en_seleccionada = None  # selección vigente si el press cayó sobre ya-seleccionado
        self._press_ya_seleccionada = False  # ¿el presionado ya estaba seleccionado antes del press?
        ...
        #
        # Se registra SIEMPRE (no solo si _REORDER_ENABLED): el árbol de
        # presupuesto (TablaArbol) no usa reorder propio, pero depende de
        # este shortcut global para su Subir/Bajar con Alt+flechas. Antes
        # el registro quedaba atado a que existiera por lo menos una tabla
        # con _REORDER_ENABLED=True (APU/generador, creadas lazy), así que
        # en una sesión con solo la pestaña de Presupuesto Alt+Up/Down no
        # hacía nada. _dispatch_alt_arrow resuelve el destino por foco.
    ...
    def add_row(self, data, parent=None, editable=True):
        ...
        # Tooltip = texto completo de la celda. Con columnas angostas o
        # contenido largo (ej. Descripción), Qt elide el texto visible con
        # "..." pero no muestra el resto en ningún lado — sin esto, la
        # única forma de leer una descripción cortada es agrandar la
        # columna a mano o entrar a editar la celda. Se aplica a todas las
        # columnas (no solo Descripción) porque cualquier tabla que herede
        # de TreeTableWidget puede tener el mismo problema con columnas
        # angostas; el costo de setear un tooltip idéntico al texto en
        # columnas cortas (ej. Unidad) es despreciable.
        for col, texto in enumerate(data):
            if texto:
                item.setToolTip(col, _tooltip_rico(texto))
    ...
    def mousePressEvent(self, event):
        # Recordar dónde inició el press — startDrag() lo usa para decidir
        # si el gesto es de arrastre o de selección (ver _puede_iniciar_drag).
        self._press_pos = event.pos()
        self._press_col = self.columnAt(event.pos().x())
        self._press_en_seleccionada = None
        self._press_ya_seleccionada = False
        self._rubber_origin = None

        item = self.itemAt(event.pos())
        sin_modificadores = not (event.modifiers() & (
            Qt.KeyboardModifier.ControlModifier | Qt.KeyboardModifier.ShiftModifier))

        # Regla de gesto (decidida UNA sola vez, en el press):
        #   - Renglón YA seleccionado → el drag arrastra TODA la selección.
        #   - Renglón NO seleccionado → el gesto es de SELECCIÓN: Qt lo
        #     selecciona (solo él) y nunca inicia drag; para moverlo hay
        #     que soltar y arrastrarlo de nuevo, ya siendo parte de la
        #     selección. Así ambos comportamientos no pueden chocar.
        if (event.button() == Qt.MouseButton.LeftButton
                and sin_modificadores
                and self._fila_destino_valida(item)):
            self._press_ya_seleccionada = item.isSelected()
        ...
        if event.button() != Qt.MouseButton.LeftButton:
            return
        item = self.itemAt(event.pos())
        # Press sobre zona vacía o fila placeholder → preparar rubber band
        # de selección (QTreeWidget no lo implementa nativamente). Si el
        # usuario solo hace clic sin arrastrar, nunca arranca y el gesto
        # sigue siendo un click normal.
        if not self._fila_destino_valida(item):
            self._rubber_origin = event.pos()
            if sin_modificadores:
                self.clearSelection()  # click en vacío: limpiar (Ctrl acumula, no limpia)
            return
        # Click sobre ya seleccionado: recordar la selección vigente —
        # Qt la colapsa al ítem presionado en el release de un click sin
        # drag, lo que mataría el drag grupal posterior (ver
        # mouseReleaseEvent).
        if (sin_modificadores
                and self._press_ya_seleccionada
                and item is not None):
            self._press_en_seleccionada = [
                it for it in self._iter_items() if it.isSelected()]
        if item.childCount() > 0 and self.columnAt(event.pos().x()) == 0:
            vrect = self.visualRect(self.indexFromItem(item))
            if event.position().x() >= vrect.x():
                self._toggle_current_expanded()

    def mouseMoveEvent(self, event):
        if self._rubber_origin is not None and (
                event.buttons() & Qt.MouseButton.LeftButton):
            # Rubber band propio activo: NO delegar a Qt. Su
            # DragSelectingState hace auto-scroll horizontal (corre las
            # filas fuera del cursor y la selección se rompe) y compite
            # con esta selección en cada move.
            ...
            if self._rubber_band is None:
                ...
                # Sin esto el rubber (widget hijo) come los mouseMove que caen
                # sobre su propia línea y el arrastre se congela.
                self._rubber_band.setAttribute(Qt.WidgetAttribute.WA_TransparentForMouseEvents, True)
            self._rubber_band.setGeometry(QRect(origin, event.pos()).normalized())
            self._rubber_band.show()
            self._seleccionar_rubber(self._rubber_band.geometry(),
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() != Qt.MouseButton.LeftButton:
            return
        # Click sobre ya seleccionado: restaurar la selección vigente que
        # Qt colapsa en este release (conserva el drag grupal).
        guardada = self._press_en_seleccionada
        self._press_en_seleccionada = None
        if guardada:
            for it in guardada:
                try:
                    ...
                    pass  # repoblado tras drag: los items viejos ya no existen
        # Cierre del rubber band
    ...
    def _iter_items(self):
        """Recorre todos los QTreeWidgetItem visibles (raíces + descendientes)."""
    ...
    def _seleccionar_rubber(self, rect: QRect, acumular: bool):
        """Selecciona los renglones cuyo rect visual intersecta el rubber band.

        acumular=False (sin Ctrl) la selección es exactamente la del rect;
        acumular=True (Ctrl) se agrega a la existente.  La fila placeholder
        y otros renglones no-destino quedan fuera siempre.
        """
        if not acumular:
            self.clearSelection()
        for it in self._iter_items():
            if not self._fila_destino_valida(it):
                continue
            r = self.visualItemRect(it)
            if r.isValid() and not r.isEmpty() and r.intersects(rect):
                it.setSelected(True)
    ...
    # _filters se usa para filtros adicionales por columna (AND).
    ...
    def filter_rows(self, text):
        ...
        También evalúa _filters si los hay."""
        if not text and not self._filters:
            self._show_all()
            return
    ...
    def _filter_item_multi(self, item, text, cols):
        ...
        match = (not text or any(text in item.text(c).lower() for c in cols)) and self._item_matches_filters(item)
    ...
    def get_column_info(self, col: int) -> dict:
        """Retorna {tipo, choices} para la columna, o default text si no está en el catálogo."""
        d = self._column_map.get(col)
        if d is None:
            return {"tipo": ColType.TEXT, "choices": None}
        return {"tipo": d.tipo, "choices": list(d.choices) if d.choices else None}
    ...
    def _puede_iniciar_drag(self) -> bool:
        """True solo si el press cayó sobre un renglón que YA estaba
        seleccionado ANTES del press — la comprobación es de PERTENENCIA
        a la selección, no de existencia.

        - Press sobre renglón no seleccionado → False: el gesto fue de
          selección (Qt lo seleccionó) y no arrastra nada; para moverlo
          hay que soltar y arrastrarlo de nuevo.
        - Press sobre zona vacía o fila placeholder → False: el gesto es
          del rubber band de selección.
        """
        if not getattr(self, "_press_ya_seleccionada", False):
            return False
        press_pos = getattr(self, "_press_pos", None)
        if press_pos is None:
            return False
        item = self.itemAt(press_pos)
        return (item is not None and self._fila_destino_valida(item)
                and item.isSelected())
    ...
    def startDrag(self, supportedActions):
        ...
        if not self._puede_iniciar_drag():
            return
        self._press_en_seleccionada = None  # el drag consumió el gesto; el release no restaura
    ...
    def copy_selection(self) -> bool:
        ...
        header = [self.headerItem().text(c) for c in cols]
        lines = ["\t".join(header)]
        for item in items:
            vals = []
            for c in cols:
                copy_val = item.data(c, COPY_ROLE)
                vals.append(copy_val if copy_val is not None else item.text(c))
    ...
    def _descartar_fila_encabezado(self, filas: list[list[str]]) -> tuple[list[list[str]], bool]:
        ...
        if not filas:
            return filas, False
        cols_visibles = [c for c in range(self.columnCount()) if not self.isColumnHidden(c)]
        encabezados = [self.headerItem().text(c) for c in cols_visibles]
