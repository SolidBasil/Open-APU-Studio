...
COLUMNAS_CATALOGO = [
    ColumnaDef(0,  "Clave",             "Identificación", tipo=ColType.TEXT,   favorita_default=True,  visible_default=False),
    ColumnaDef(1,  "Descripción",       "Identificación", tipo=ColType.TEXT,   favorita_default=True,  visible_default=True),
    ColumnaDef(2,  "Unidad",            "Identificación", tipo=ColType.CHOICE, choices=UNIDADES,        favorita_default=True,  visible_default=True),
    ColumnaDef(4,  "Tipo",              "Identificación", tipo=ColType.CHOICE, choices=("Material", "Mano de obra", "Herramienta", "Equipo", "Auxiliar", "Concepto", "Flete", "Trabajo"), favorita_default=True,  visible_default=True),
    ColumnaDef(5,  "Familia",           "Identificación", tipo=ColType.TEXT,   favorita_default=True,  visible_default=True),
    ColumnaDef(6,  "Proveedor",         "Identificación", tipo=ColType.TEXT,   favorita_default=True,  visible_default=True),
    ColumnaDef(13, "Clave Usuario",     "Identificación", tipo=ColType.TEXT,   favorita_default=False, visible_default=False),
    ColumnaDef(14, "Compuesto",         "Identificación", tipo=ColType.CHOICE, choices=("Sí", "No"),    favorita_default=False, visible_default=False),

    ColumnaDef(8,  "Desc. Corta",       "Descripción",    tipo=ColType.TEXT,   favorita_default=True,  visible_default=False),

    ColumnaDef(3,  "Precio",            "Costos",         tipo=ColType.NUMERIC, favorita_default=True,  visible_default=True),
    ColumnaDef(7,  "F. Precio",         "Costos",         tipo=ColType.DATE,    favorita_default=True,  visible_default=False),
    ColumnaDef(9,  "Costo MN",          "Costos",         tipo=ColType.NUMERIC, favorita_default=True,  visible_default=False),
    ColumnaDef(10, "Costo ME",          "Costos",         tipo=ColType.NUMERIC, favorita_default=True,  visible_default=False),
    ColumnaDef(12, "Costo Directo",     "Costos",         tipo=ColType.NUMERIC, favorita_default=False, visible_default=False),

    ColumnaDef(15, "Usar Hoja FASAR",   "Mano de obra",   tipo=ColType.CHOICE, choices=("Sí", "No"),    favorita_default=False, visible_default=False),
    ColumnaDef(16, "Factor FSR",        "Mano de obra",   tipo=ColType.NUMERIC, favorita_default=False, visible_default=False),

    ColumnaDef(17, "Tipo de Trabajo",   "Trabajo",        tipo=ColType.TEXT,   favorita_default=False, visible_default=False),

    ColumnaDef(18, "Peso (kg)",         "Datos adicionales", tipo=ColType.NUMERIC, favorita_default=False, visible_default=False),
    ColumnaDef(19, "Comentarios",       "Datos adicionales", tipo=ColType.TEXT,   favorita_default=False, visible_default=False),

    ColumnaDef(11, "Hash",              "Auditoría",      tipo=ColType.TEXT,   favorita_default=True,  visible_default=False),
    ColumnaDef(20, "Creado",            "Auditoría",      tipo=ColType.DATE,   favorita_default=False, visible_default=False),
    ColumnaDef(21, "Creado Por",        "Auditoría",      tipo=ColType.TEXT,   favorita_default=False, visible_default=False),
    ColumnaDef(22, "Modificado",        "Auditoría",      tipo=ColType.DATE,   favorita_default=False, visible_default=False),
    ColumnaDef(23, "Modificado Por",    "Auditoría",      tipo=ColType.TEXT,   favorita_default=False, visible_default=False),
]
...
class TablaInsumos(TreeTableWidget):
    ...
    def _add_empty_row(self):
        """Fila visual vacía al final — comportamiento tipo Excel: se puede
        escribir directo en sus celdas (Descripción, Unidad, Precio, Tipo,
        Familia, igual que en cualquier fila real). Al confirmar una
        Descripción no vacía, _on_insumo_editado (paneles.py) crea el
        insumo real con lo que se haya escrito hasta ese momento y agrega
        una fila vacía nueva después — igual que Excel/Sheets, donde la
        última fila siempre está lista para capturar.

        Antes esta fila tenía editable=False y un solo clic abría un
        diálogo modal (\"Nuevo insumo\") — funcionaba más como un botón que
        como una fila de tabla."""
        item = self.add_row(
            ["", "Nuevo insumo...", "", "", "", "", "", "", "", "",
             "", "", "", "", "", "", "", "", "", "", "", "", "", ""],
            editable=True,
    ...
    def _al_click_fila_vacia(self):
        ...
        abre el diálogo modal. Crear el insumo ocurre al escribir una
        Descripción (ver _on_insumo_editado en paneles.py)."""
        pass
