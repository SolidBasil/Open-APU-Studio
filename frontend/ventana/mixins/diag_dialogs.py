...
class DiagDialogsMixin:
    ...
    def _on_recalcular(self):
        ...
        try:
            with progreso_indeterminado(self, "Recalculando presupuesto…"):
                resultado = self._api.recalcular_proyecto()
        except Exception as e:
            from frontend.ventana.ui_utils import mostrar_error
            mostrar_error(self, "Error al recalcular", e)
            return

        n_iter = resultado.get("iteraciones_compuestos", 0)
        convergio = resultado.get("convergio", True)  # True si el backend no lo reporta (ej. HTTP)
        if not convergio:
            # Hallazgo 8: antes esto no se distinguía de una convergencia
            # normal — se mostraba "recalculado (15 iteraciones)" igual
            # que si hubiera convergido justo en la última. Ahora se avisa
            # explícitamente que los valores finales podrían no ser el
            # resultado en equilibrio (típicamente insumos compuestos
            # anidados en un ciclo, ej. A usa B y B usa A).
            QMessageBox.warning(
                self, "El recálculo no convergió",
                f"Se alcanzó el límite de {n_iter} iteraciones sin que los "
                "costos de insumos compuestos se estabilizaran.\n\n"
                "Los totales mostrados pueden no ser el resultado final en "
                "equilibrio. Esto suele pasar cuando un insumo compuesto "
                "termina usándose a sí mismo indirectamente (A usa B, B "
                "usa A). Revisa las matrices de insumos compuestos "
                "recientes.",
            )
    ...
    def _on_depurar_catalogos(self):
        ...
        if not total:
            self._sb.showMessage("Catálogo limpio — no se encontraron inconsistencias.", 4000)
            return
    ...
    def _on_homologar_hash(self):
        ...
        if not cambios:
            self._sb.showMessage("Todos los insumos tienen su hash correcto.", 4000)
            return
    ...
    def _on_info_proyecto(self, nuevo=False):
        ...
        def _add_field(form: QFormLayout, key: str, label: str,
            ...
            if widget_type == "line":
                ...
                # Para obra_latitud/obra_longitud — a diferencia de
                # spin_float, sí permite negativos: México está al oeste
                # de Greenwich, así que su longitud real es negativa (ej.
                # CDMX ≈ -99.13). El rango de spin_float (0, 999999) no
                # solo bloqueaba capturar un valor negativo — también
                # recortaba en silencio a 0 cualquier valor negativo que
                # ya existiera en la BD al mostrarlo (QDoubleSpinBox
                # recorta setValue() al rango del widget). Ver hallazgo N7
                # del seguimiento. El rango aquí es generoso a propósito
                # (cubre tanto latitud ±90 como longitud ±180); el límite
                # exacto por campo ya lo aplica SchemaRegistry al guardar.
                w = QDoubleSpinBox()
                w.setDecimals(6)
                w.setRange(-180, 180)
                w.setValue(float(value) if value else 0)
        ...
        _add_field(form, "obra_latitud", "Latitud:", datos.get("obra_latitud"), "spin_coord")
        _add_field(form, "obra_longitud", "Longitud:", datos.get("obra_longitud"), "spin_coord")
