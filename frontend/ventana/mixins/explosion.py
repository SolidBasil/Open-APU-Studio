...
class ExplosionMixin:
    ...
    def _build_explosion(self):
        ...
        if not concepto_ids:
            self._sb.showMessage("No hay conceptos en el presupuesto para explotar.", 4000)
            return None
        ...
        if not filas:
            self._sb.showMessage("No se encontraron insumos con los filtros seleccionados.", 4000)
            return None
    ...
    def _build_matriz_explosion(self):
        ...
        if not concepto_ids:
            concepto_ids = self._api.todos_concepto_ids()
        if not concepto_ids:
            self._sb.showMessage("No hay conceptos en el presupuesto.", 4000)
            return None
        ...
        if total_conceptos == 0:
            self._sb.showMessage("Los conceptos seleccionados no tienen APU.", 4000)
            return None
