...
class ApuMixin:
    ...
    def _build_apu_tab(self, matriz_id: int, descripcion: str = "", *,
        ...
        from frontend.ventana.tipos_insumo import ICONO_SVG as _INS_ICONO_SVG
        ...
        def _construir_filtros(subtotales: dict):
            ...
            for tid in sorted(subtotales.keys()):
                ...
                btn = QPushButton(f"{nombre}  ${subtotal:,.2f}")
                btn.setIcon(icono(_INS_ICONO_SVG.get(tid, "file-text"), 14))
    ...
    def _on_concepto_editado(self, item, column):
        ...
        """
        from frontend.ventana.widgets.arbol import ID_ROLE, EMPTY_ROLE
    ...
    def _on_fila_vacia_editada(self, item, column):
        """Escribir en la fila vacía final del Presupuesto agrega un
        concepto nuevo — comportamiento tipo Excel, igual que se hizo para
        Insumos (ver _on_insumo_fila_vacia_editada en paneles.py).

        A diferencia de Insumos, un concepto no es texto libre: siempre
        tiene que referenciar un insumo real del catálogo (ver
        _resolver_insumo_pegado en arbol.py, que ya documenta esta misma
        regla para pegado — "nunca se inventa un insumo nuevo ni se deja
        el concepto con datos huérfanos"). Por eso acá NO se crea nada a
        ciegas con lo escrito:

          - Si el texto matchea EXACTO (sin importar mayúsculas) la
            descripción de un único insumo del catálogo, se usa ese
            insumo directo — sin abrir ningún diálogo (el autocompletado
            de _crear_editor_descripcion en arbol.py hace que llegar a un
            match exacto sea rápido: son sugerencias, no obligación).
          - Si no matchea ninguno (o el texto es solo un fragmento),
            se abre el selector de insumo normal, pero con ese texto ya
            precargado como búsqueda — así no se pierde lo ya escrito y
            no hay que volver a teclearlo en un diálogo en blanco.

        No hace falta convertir el item a mano ni resetearlo: agregar_nodo
        dispara ProyectoRecalculado, y TablaArbol ya repuebla el árbol
        completo desde la fuente de verdad al recibirlo (ver
        _on_proyecto_recalculado en arbol.py) — eso ya regenera la fila
        vacía sola, igual que como si se hubiera usado el diálogo de
        siempre.
        """
        if column != 4 or not self._api:
            return
        texto = item.text(4).strip()
        if not texto:
            return
        ...
        try:
            candidatos = [
                i for i in self._api.insumos()
                if (i.get("descripcion") or "").strip().lower() == texto.lower()
            ]
        except Exception:
            candidatos = []
        if len(candidatos) == 1:
            insumo_id = candidatos[0].get("id")

        if insumo_id is not None:
            self._agregar_nodo("concepto", insumo_id=insumo_id)
        else:
            self._agregar_nodo("concepto", busqueda_inicial=texto)
    ...
    def _on_agregar_componente_apu(self, matriz_id: int, *, insumo_id: int | None = None,
                                    busqueda_inicial: str = ""):
        """Agrega un insumo como componente nuevo al APU de `matriz_id`.

        `insumo_id` ya resuelto salta el diálogo por completo — lo usa
        TablaApuDetalle._on_fila_vacia_editada (widgets/apu.py) cuando
        escribir en la fila vacía final ya matcheó una descripción exacta
        del catálogo. Sin eso, abre el selector de siempre (con
        `busqueda_inicial` precargada si se da)."""
