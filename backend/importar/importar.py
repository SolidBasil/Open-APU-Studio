...
def _leer_dbf(ruta: Path, encoding: str = "cp850") -> list[dict]:
    """Lee un .DBF devolviendo solo registros activos (_deleted=False).

    Se prueba cp850 primero — es la codificación real que usan los DBF
    que genera OPUS/Neodata para texto en español (acentos, ñ). Antes se
    probaba latin-1 primero (con cp850 solo como "fallback" si latin-1
    lanzaba una excepción). El problema: latin-1 (ISO-8859-1) asigna un
    carácter a cada uno de los 256 valores de byte posibles, así que
    decodificar con latin-1 casi nunca lanza UnicodeDecodeError — "tiene
    éxito" incluso en archivos que en realidad están en cp850, solo que
    con los acentos/ñ mal decodificados en silencio. En la práctica, el
    fallback a cp850 casi nunca llegaba a ejecutarse (Hallazgo 10 de la
    auditoría). Se prueba primero cp850 (encoding correcto de origen),
    luego el `encoding` explícito si el caller pidió otro distinto, y
    latin-1 solo como último recurso genérico.
    """
    if not ruta or not ruta.exists():
        return []
    intentos = list(dict.fromkeys(["cp850", encoding, "latin-1"]))
    last_err = None
    for enc in intentos:
        try:
            ...
            print(f"  [OK] {ruta.name}: {len(activos)} activos / {borrados} borrados (encoding={enc})")
            return activos
...
def _padre_por_wbs(wbs: str, wbs_a_id: dict) -> tuple[int | None, bool]:
    """
    ...
    LIMITACIÓN CONOCIDA (Hallazgo 9 de la auditoría): el truncado es
    carácter por carácter, sin ninguna noción real de "segmento" o nivel.
    Si el WBS de un proyecto usa segmentos de más de un dígito por nivel
    (irregular), truncar un solo carácter puede coincidir por casualidad
    con el WBS de un nodo que NO es el ancestro real. No hay forma de
    distinguir ese caso de un salto de nivel legítimo (un nivel
    intermedio que nunca se importó como nodo propio) sin conocer la
    convención exacta del proyecto de origen — así que en vez de
    adivinar, esta función reporta cuándo tuvo que truncar MÁS de un
    carácter para encontrar coincidencia, vía el segundo valor devuelto
    (`ambiguo`), para que el import pueda avisar y el resultado se
    revise a mano en vez de confiar en silencio.

    Devuelve (padre_id, ambiguo). `ambiguo` es True si hizo falta
    truncar más de un carácter para encontrar el padre (señal de
    jerarquía potencialmente irregular — revisar manualmente).
    """
    ...
    while codigo:
        if codigo in wbs_a_id:
            return wbs_a_id[codigo], (truncados > 1)
        codigo = codigo[:-1]
        truncados += 1
    return None, False
...
def importar(
    ...
    # ── Árbol ─────────────────────────────────────────────────────────────
    wbs_stats = {"wbs": 0, "sin_resolver": 0, "wbs_ambiguo": 0}
    if formato == "numerico":
        nodo_id_sqlite, clave_a_conceptos, wbs_stats = _arbol_numerico(
            con, cur, proyecto_id, regs_1, regs_a, regs_p)
    else:
        # _arbol_clasico no usa truncado de WBS (arma la jerarquía desde
        # CLAVENUM), así que no tiene wbs_ambiguo que reportar — se queda
        # en 0 (default de wbs_stats de arriba).
    ...
    stats = {
        ...
        # Hallazgo 9 de la auditoría: antes esta señal solo se imprimía en
        # consola (print, no siempre visible para el usuario de la app de
        # escritorio) y se perdía — no llegaba al dict que esta función sí
        # devuelve. "wbs_ambiguo" > 0 significa que hubo nodos cuyo padre
        # se determinó truncando MÁS de un carácter del WBS — señal de
        # jerarquía potencialmente irregular que vale la pena revisar a
        # mano (ver _padre_por_wbs).
        "wbs_sin_resolver": wbs_stats["sin_resolver"],
        "wbs_ambiguo":      wbs_stats["wbs_ambiguo"],
    }
    print("\n--- Resumen ---")
...
def _arbol_numerico(con, cur, proyecto_id, regs_1, regs_a, regs_p) -> dict:
    ...
    stats = {"wbs": 0, "sin_resolver": 0, "wbs_ambiguo": 0}
    ...
    for r in nodos_ord:
        ...
        if pre_idpad == -1:
            ...
            padre_id, wbs_ambiguo = _padre_por_wbs(wbs, wbs_a_sqlite)
        if padre_id is None:
    ...
    print(f"    WBS: {stats['wbs']}  |  "
          ...
          f"Ambiguos (revisar): {stats['wbs_ambiguo']}")
    con.commit()
    return nodo_id_sqlite, clave_nodo_ids, stats
