...
class RecalculoRepo(RepoBase):
    ...
    def recalcular_proyecto(self, proyecto_id: int) -> dict:
        ...
        # Si `cambios` sigue siendo True al salir del while, fue porque se
        # llegó a MAX_ITERACIONES sin que los costos de insumos compuestos
        # se estabilizaran — no convergió, y los valores finales pueden no
        # ser el resultado en equilibrio (típicamente insumos compuestos
        # anidados en un ciclo, ej. A usa B y B usa A). Antes de este fix,
        # `iteraciones_compuestos` se devolvía pero nada lo comparaba
        # contra MAX_ITERACIONES en ningún lado — ni la única pantalla que
        # lo mostraba (diag_dialogs.py) distinguía "convergió en 15" de
        # "se cortó en 15 sin converger" (Hallazgo 8 de la auditoría).
        convergio = not cambios
