...
class ApuMatricesRepo(RepoBase):
    ...
    def con_formula_por_proyecto(self, proyecto_id: int) -> list[dict]:
        """Componentes APU con `formula` no vacía, de cualquier matriz que
        pertenezca a este proyecto (respeta la convención de signo de
        matriz_id: positivo = estructura_presupuesto.id, negativo =
        -insumo_id de un insumo compuesto).

        Usado por Api.variables_eliminar() para encontrar fórmulas que
        referencian una variable antes de borrarla (Hallazgo 5)."""
        return self._lista("""
            SELECT ac.id, ac.matriz_id, ac.formula
            FROM apu_matrices ac
            WHERE ac.formula IS NOT NULL AND ac.formula != ''
              AND (
                    (ac.matriz_id > 0 AND ac.matriz_id IN (
                        SELECT id FROM estructura_presupuesto WHERE proyecto_id = ?
                    ))
                 OR (ac.matriz_id < 0 AND -ac.matriz_id IN (
                        SELECT id FROM insumos WHERE proyecto_id = ?
                    ))
              )
