...
def crear_registry(db: Database) -> RepositoryRegistry:
    ...
    from backend.database.repos import (
        ...
        GeneradorRepo, VariableFormulaRepo, IndirectoRepo,
    )
    from backend.database.repos.generador import GeneradorRenglonRepo
    ...
    registry.registrar("generador_renglones", GeneradorRenglonRepo)
    registry.registrar("variables_formula", VariableFormulaRepo)
    registry.registrar("indirectos", IndirectoRepo)
    return registry
