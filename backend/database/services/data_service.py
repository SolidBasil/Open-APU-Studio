...
_EVENTO_POR_ENTIDAD: dict[str, type[Evento]] = {
    ...
    "indirectos": IndirectoActualizado,
}
...
#
# Antes de que insertar() capturara historial de creación (ver Hallazgo 3),
# esta lista solo se usaba para los pocos flujos que llamaban a
# capturar_creado() a mano (duplicar_bloque, pegar componente APU, duplicar
# renglón de generador) y por eso solo tenía 2 de las 7 tablas reales con
# columna 'activo'. Ahora que insertar() la llama para TODA entidad, hacía
# falta completarla — si no, crear un insumo/familia/subfamilia/indirecto/
# generador nuevo y deshacerlo hacía un DELETE físico, y rehacer no podía
# revivirlo (redo silenciosamente no hacía nada).
_TABLAS_CON_SOFT_DELETE = {
...
class DataService:
    ...
    def insertar(self, entidad: str, usuario_id: int = 1,
        ...
        Antes de este fix, insertar() no llamaba a
        HistorialRepo.capturar_creado() — ninguna fila creada desde este
        método genérico era deshacible con Ctrl+Z (Hallazgo 3 de la
        auditoría). Los pocos flujos que sí necesitaban poder deshacer una
        creación (duplicar_bloque, pegar componente APU, duplicar renglón
        de generador) ya llamaban a capturar_creado() a mano, bypasseando
        este método directamente — eso sigue igual, no se duplica la
        captura para ellos.
    ...
    def deshacer(self, usuario_id: int = 1,
        ...
        with self._db.transaction():
            for c in cambios:
                ...
                # asignar) — no "no hay nada que restaurar". Cada `c` en
                # `cambios` viene de cambios_sesion(), así que siempre es
                # una captura real; saltarla con `continue` dejaba el
                # campo con su valor actual en vez de restaurar el NULL
                # (bug encontrado auditando N6 — reasignar_generador()
                # capturaba bien el historial, pero deshacer() no podía
                # aplicarlo de vuelta cuando el valor anterior era None).
                if c["tabla"] in ("insumos", "estructura_presupuesto", "apu_matrices", "generador_renglones") and valor is not None:
                    try:
                        valor = float(valor)
                    except (ValueError, TypeError):
                        pass
            ...
            # Si el undo tocó la jerarquía (padre_id/orden revertidos),
            # reindexar() es quien regenera wbs/nivel coherentes — sin esto
            # el árbol quedaba con wbs viejos mezclados (1.1, 1.2, 1.8, 1.5).
            if any(c["tabla"] == "estructura_presupuesto" for c in cambios):
                from backend.database.repos import NodoRepo
                NodoRepo(self._db.conn).reindexar(pid)
        ...
        with self._db.transaction():
            for c in cambios:
                ...
                if c["campo"] == CAMPO_CREADO:
                    ...
                    continue
                valor = c["valor_nuevo"]
                # Mismo caso que en deshacer(): valor_nuevo puede ser
                # legítimamente NULL, no significa "nada que rehacer".
                if c["tabla"] in ("insumos", "estructura_presupuesto", "apu_matrices", "generador_renglones") and valor is not None:
                    try:
                        valor = float(valor)
                    except (ValueError, TypeError):
            ...
            self._recalcular_generadores_tocados(cambios)
            # Mismo caso que deshacer(): restaurar padre_id/orden exige
            # regenerar wbs/nivel del árbol completo.
            if any(c["tabla"] == "estructura_presupuesto" for c in cambios):
                from backend.database.repos import NodoRepo
                NodoRepo(self._db.conn).reindexar(pid)
            RecalculoRepo(self._db.conn).recalcular_proyecto(pid)
    ...
    def guardar_renglon_generador(self, generador_id: int,
        ...
        # Antes de este fix, este método (a diferencia de insertar()/
        # actualizar() genéricos) nunca llamaba a SchemaRegistry — las
        # reglas de "generador_renglones" (largo/ancho/alto >= 0) eran
        # letra muerta para el único camino real de escritura que usa la
        # UI (Hallazgo 2 de la auditoría).
        try:
            self._schema.validate("generador_renglones", campos)
        except ValidationError:
            raise
        except Exception as e:
            raise DataServiceError(str(e)) from e
        ...
        # Antes de este fix, este método (a diferencia de insertar()/
        # actualizar() genéricos, y a diferencia de
        # eliminar_renglon_generador() más abajo, que ya se corrigió por
        # el mismo motivo) nunca capturaba historial — ni crear ni editar
        # un renglón de generador era deshacible con Ctrl+Z, aunque
        # eliminarlo sí lo era desde otro fix anterior. Mismo patrón que
        # usan actualizar()/insertar() genéricos: por campo cambiado en
        # edición, capturar_creado() en alta.
        from backend.database.repos.historial import HistorialRepo
        ...
        try:
            with self._db.transaction():
                ...
                if renglon_id:
                    for campo, nuevo_valor in campos.items():
                        viejo = h_repo.valor_campo("generador_renglones", renglon_id, campo)
                        if str(viejo) != str(nuevo_valor):
                            h_repo.capturar(
                                tabla="generador_renglones", registro_id=renglon_id,
                                campo=campo, valor_anterior=viejo, valor_nuevo=nuevo_valor,
                                usuario_id=usuario_id, sesion=sesion,
                            )
                    ...
                    h_repo.capturar_creado("generador_renglones", renglon_id,
                                            usuario_id=usuario_id, sesion=sesion)
    ...
    def eliminar_renglon_generador(self, renglon_id: int,
        ...
        Encontrado auditando los métodos custom de DataService (hallazgo
        N6 del seguimiento): a diferencia del eliminar() genérico, este
        método nunca capturaba historial — borrar un renglón de generador
        no era deshacible con Ctrl+Z, aunque la tabla sí soporta
        soft-delete (está en _TABLAS_CON_SOFT_DELETE). Se agregó la
        captura, mismo patrón que usa eliminar().
        """
        from backend.database.repos.generador import GeneradorRepo
        from backend.database.repos.recalculo import RecalculoRepo
        from backend.database.repos.historial import HistorialRepo
        ...
        try:
            with self._db.transaction():
                ...
                sesion = self._sesion or str(uuid.uuid4())
                try:
                    viejo = h_repo.valor_campo("generador_renglones", renglon_id, "activo")
                except Exception:
                    viejo = None
    ...
    def reasignar_generador(self, generador_id: int,
        ...
        Encontrado auditando los métodos custom (N6 del seguimiento):
        escribía directo vía gen_repo.update() sin capturar historial —
        reasignar un generador a otro concepto no era deshacible con
        Ctrl+Z. Se agregó la captura, mismo patrón que actualizar()."""
        from backend.database.repos.generador import GeneradorRepo
        from backend.database.repos.recalculo import RecalculoRepo
        from backend.database.repos.historial import HistorialRepo
        ...
        try:
            with self._db.transaction():
                h_repo = HistorialRepo(self._db.conn)
                sesion = self._sesion or str(uuid.uuid4())
                # OJO: valor_campo() devuelve None tanto si la fila/columna
                # no existe como si el valor es legítimamente NULL (ej.
                # concepto_id sin asignar) — un simple "is not None" se
                # saltaría la captura justo en ese caso real. Se usa un
                # sentinel aparte para distinguir "no se pudo leer" de
                # "se leyó y es None".
                capturar_ok = True
                try:
                    viejo = h_repo.valor_campo("generadores", generador_id, "concepto_id")
                except Exception:
                    viejo = None
                    capturar_ok = False
                ...
                if capturar_ok:
                    h_repo.capturar(
                        tabla="generadores", registro_id=generador_id,
                        campo="concepto_id", valor_anterior=viejo,
                        valor_nuevo=nuevo_concepto_id,
                        usuario_id=usuario_id, sesion=sesion,
                    )
    ...
    def _evento(self, entidad: str, id: int, cambios: dict,
        ...
        if entidad == "indirectos":
            # `id` aquí es el id del indirecto, no el proyecto — el evento
            # necesita proyecto_id para que el panel sepa qué refrescar.
