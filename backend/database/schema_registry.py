...
class SchemaRegistry:
    ...
    _rules: dict[str, dict[str, FloatField | IntField | StringField | BoolField]] = {
        ...
        "generadores": {
            "nombre": StringField(),
            "unidad": StringField(),
            "cantidad_total": FloatField(),  # es un SUM(subtotal) recalculado; puede
                                              # dar negativo si hay renglones con
                                              # 'veces' negativo (deducciones) — no
                                              # se restringe aquí, ver 'veces' abajo.
        },
        "generador_renglones": {
            # 'largo'/'ancho'/'alto' son dimensiones físicas: nunca son
            # negativas en ninguna convención de generador de cantidades.
            # Antes de este fix no había ninguna regla aquí — se podían
            # capturar valores negativos y se propagaban sin error hasta
            # el total del presupuesto (Hallazgo 2 de la auditoría).
            "largo": FloatField(min=0),
            "ancho": FloatField(min=0),
            "alto": FloatField(min=0),
            # 'veces' (el multiplicador/conteo) NO se restringe a >= 0 a
            # propósito: en generadores de obra al estilo Opus/Neodata es
            # práctica común usar 'veces' negativo para restar huecos de
            # puertas/ventanas del área de un muro (deducciones). No hay
            # evidencia en este repo de que esa convención esté soportada
            # explícitamente hoy, pero tampoco de que esté descartada —
            # así que no se bloquea a ciegas. Si el negocio confirma que
            # las deducciones NO se manejan así en este proyecto, agregar
            # aquí `FloatField(min=0)` para 'veces' también.
            "subtotal": FloatField(),
        },
    }
